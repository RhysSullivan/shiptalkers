...
export async function GET(req: Request) {
  ...
  const percentageTweets = ((tweets / commits) * 100).toFixed();
  const percentageCommits = ((commits / tweets) * 100).toFixed();
  const txt =
    tweets > commits
  ...
  const barHeight = 500;
  const tweetBarHeight = barHeight * (tweets / (tweets + commits));
  const commitBarHeight = barHeight * (commits / (tweets + commits));
