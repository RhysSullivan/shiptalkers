...
export async function writeToCache<T>(key: string, value: T, EX?: number) {
    if (EX) {
        return client.set(key, superjson.stringify(value), { EX });
    }
    return client.set(key, superjson.stringify(value));
}
